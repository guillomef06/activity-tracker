...
- **Score total dans la liste des inscriptions admin** ✅ : chaque ligne d'inscription (`mg-admin-tab`) affiche désormais le score total courant du joueur, à côté de son pseudo.
...
- **`angular-cli-ghpages` 2→3.1.0** (devDependency, script `npm run deploy` — le déploiement prod réel passe par `actions/deploy-pages` en CI, non affecté) : résout la chaîne de vulnérabilités critique/high `gh-pages`/`picomatch`/`ajv`/`@angular-devkit/core` (`npm audit` : 16 → 8 findings). Seule dépendance directe touchée, aucune option custom dans `angular.json` à migrer, aucun breaking change applicable (v3 exige Angular 18+, déjà sur 22). `npm audit fix` appliqué ensuite pour dédupliquer les paquets imbriqués restants — aucun range semver modifié en dehors de `angular-cli-ghpages` lui-même. Findings restants : `xlsx` (voir `SPEC_XLSX_SECURITY_FIX.md`) et la chaîne `webpack-dev-server`/`uuid`/`sockjs` (dev-server local uniquement, pas de fix disponible)
- **`xlsx` CVEs corrigés** (GHSA-4r6h-8v6p-xvw6 prototype pollution + GHSA-5pgg-2g8v-p4x9 ReDoS, `import-excel-tab.component.ts`) : `0.18.5` → `0.20.3` installé depuis le CDN officiel SheetJS (`https://cdn.sheetjs.com/xlsx-0.20.3/xlsx-0.20.3.tgz`, plus publié sur npm après `0.18.5`) ; disparaît de `npm audit` car source non-registry, attendu
