...
describe('Date Utility Functions', () => {
  describe('getWeekStart', () => {
    it('should return Monday for a date in the middle of the week', () => {
      const wednesday = new Date('2026-02-11T12:00:00Z'); // Wednesday UTC
      const monday = getWeekStart(wednesday);

      expect(monday.getUTCDay()).toBe(1); // Monday = 1
      expect(monday.getUTCDate()).toBe(9); // Feb 9, 2026 is Monday
    });
    ...
    it('should return the previous Monday for a Sunday', () => {
      ...
      expect(result.getUTCDate()).toBe(2); // Feb 2, 2026
    });
  ...
  describe('getWeekEnd', () => {
    it('should return Sunday for a date in the middle of the week', () => {
      const wednesday = new Date('2026-02-11T12:00:00Z');
      const sunday = getWeekEnd(wednesday);

      expect(sunday.getUTCDay()).toBe(0); // Sunday = 0
      expect(sunday.getUTCDate()).toBe(15); // Feb 15, 2026 is Sunday
    });
    ...
    it('should return the same date if already Sunday', () => {
      ...
      expect(result.getUTCDate()).toBe(8); // Feb 8, 2026
    });
  });
  ...
  describe('Activity Availability Integration Test', () => {
    ...
    it('should correctly identify available activities for week 3', () => {
      // Week 3 = Feb 9–15, 2026. Use Feb 11 (Wednesday) UTC.
    ...
    it('should correctly identify available activities for week 4', () => {
      // Week 4 = Feb 16–22, 2026. Use Feb 18 (Wednesday) UTC.
